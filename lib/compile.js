...
function bundleFile(path, options, cache) {
  return browserify({entries: [path], noParse: options.noParse, extensions: options.extensions, resolve: options.resolve, basedir: options.basedir,
   insertGlobals: options.insertGlobals, detectGlobals: options.detectGlobals, ignoreMissing: options.ignoreMissing, debug: options.debug,
   standalone: options.standalone || false, cache: (options.cache === 'dynamic' ? cache.getCache() : {}) });
}
function bundleModule(modules, options, cache) {
...
function compile(path, options, cb, cacheUpdated) {
  ...
  var b = Array.isArray(path) ? bundleModule(path, options, cache) : bundleFile(path, options, cache);
  if (options.cache === 'dynamic') {
